...
* Progress & Contributions:
    ...
    * Joyce:
        1) Joyce has crawled code base from Google Cloud Platform repos and then implemented the Retrieval-Augmented Generation (RAG) functionality with crawled code base as the retrieval knowledge on "Code Generation" page using langchain. However, it did not work well because: first, Python is very popular, and the backbone LLM must have been trained a lot on Python data, and second, the code base has too much noise, not just focusing on Python but on every topic like multi-modal models. We might try out with different embeddings in the future, so we decided to left the code in place but commented out.
        2) Moreover, as mentioned from Tess, we organized code into functions and classes for all chatbot pages. Specifically, I processed 'Code Generation' page, process all chatbot message statements and move utility functions to 'src/chatbot_utilities.py'. 
        3) Lastly, I modified LLM Chatbot's backend prompt with one more layer so that if the user is a Python beginner, the chatbot can produce tutorials on Python. Otherwise, it will detect user's intent and provide answers regarding user's prompt.
