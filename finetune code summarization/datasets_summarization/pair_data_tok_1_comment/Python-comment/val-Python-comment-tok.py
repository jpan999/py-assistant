...
freq = np . zeros ( 26 , dtype = np . int ) NEW_LINE
...
def __init__ ( self , data ) : NEW_LINE INDENT self . key = data NEW_LINE self . left = None NEW_LINE self . right = None NEW_LINE DEDENT
def insert ( node , key ) : NEW_LINE
if ( node == None ) : NEW_LINE INDENT return newNode ( key ) NEW_LINE DEDENT
if ( key < node . key ) : NEW_LINE INDENT node . left = insert ( node . left , key ) NEW_LINE DEDENT elif ( key > node . key ) : NEW_LINE INDENT node . right = insert ( node . right , key ) NEW_LINE DEDENT
return node NEW_LINE
def findMaxforN ( root , N ) : NEW_LINE
while ( root != None and root . right != None ) : NEW_LINE
if ( N > root . key and N >= root . right . key ) : NEW_LINE INDENT root = root . right NEW_LINE DEDENT
elif ( N < root . key ) : NEW_LINE INDENT root = root . left NEW_LINE DEDENT else : NEW_LINE INDENT break NEW_LINE DEDENT if ( root == None or root . key > N ) : NEW_LINE print ( - 1 ) NEW_LINE else : NEW_LINE print ( root . key ) NEW_LINE
